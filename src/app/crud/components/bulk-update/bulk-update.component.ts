...
export class BulkUpdateComponent implements OnInit {
  ...
  getForm(){
    this.subscriptionsList.push(
      this.crudSrv.getDataToUpdate().subscribe((data: any) => {
        if (data) {
  ...
  updateRecords(){
    this.customerForm.value["customerFormArray"].forEach(data => {            
      this.subscriptionsList.push(
        this.crudSrv.bulkUpdateData(data).subscribe((data: any) => {
  ...
  bulkDelete(){
    ...
    this.subscriptionsList.push(
      this.crudSrv.bulkDeleteData(this.strCSVId).subscribe((data: any) => {       
